...
object SparkGZDemo {
  def main(args: Array[String]): Unit = {
    ...
    val rdd = sc.textFile("file:///home/ubuntu/bigData-temp/*/*.gz")
      ...
      }).coalesce(3,true)       //保存到hdfs前进行coalesce操作，指定分片数，可以避免过多的空文件的产生
    ...
    //—————————————————————————处理info.log.gz—————————————————————————————————————
      ...
      }).coalesce(3,true).saveAsTextFile("hdfs://hadoop72:9820/info/"+UUID.randomUUID().toString().replaceAll("-",""))
    //—————————————————————————处理error.log.gz—————————————————————————————————————
      ...
      }).coalesce(3,true).saveAsTextFile("hdfs://hadoop72:9820/error/"+UUID.randomUUID().toString().replaceAll("-",""))
    //—————————————————————————处理websocket.log.gz—————————————————————————————————————
      ...
      }).coalesce(3,true).saveAsTextFile("hdfs://hadoop72:9820/websocket/"+UUID.randomUUID().toString().replaceAll("-",""))
  }
}
